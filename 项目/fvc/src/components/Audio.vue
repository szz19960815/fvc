		...
		methods:{
			...
			loadMore(){
				...
				this.timer = setTimeout(()=>{
					let page = store.state.AUDIO.length/20;
//					console.log(page)
					store.dispatch('moreAudioAction',page+1);
					this.loadLogo = false;
				},800)
			},
			zan(index,e){
				e = event || ''
				store.state.AUDIO[index].love = Number(store.state.AUDIO[index].love)+1;
				e.target.setAttribute('style','color:red');
				store.state.like.push(store.state.AUDIO[index])
			},
			cai(index,e){
				e = event || ''
				store.state.AUDIO[index].hate = Number(store.state.AUDIO[index].hate)+1;
				e.target.setAttribute('style','color:red');
			}
		},
		computed:{
