<template>
	...
	<div id="listContent">
		<mt-header fixed title="关注"></mt-header>
		...
		methods:{
			...
			loadMore(){
				...
				this.timer = setTimeout(()=>{
					let page = store.state.like.length/20;
//					console.log(page)
			...
			imagesInfo(id){
				...
				store.state.like.map((data,i)=>{
					if(data.id === id){
//						console.log(data.name)
						this.Img = store.state.like[i]
//						console.log(this.Img)
			...
			articleInfo(id){
				...
				store.state.like.map((data,i)=>{
					if(data.id === id){
//						console.log(data.name)
						this.Article = store.state.like[i]
//						console.log(this.Article)
			...
			zan(index,e){
				e = event || ''
				store.state.like[index].love = Number(store.state.like[index].love)+1;
				e.target.setAttribute('style','color:red');
				store.state.like.push(store.state.like[index])
			},
			cai(index,e){
				e = event || ''
				store.state.like[index].hate = Number(store.state.like[index].hate)+1;
				e.target.setAttribute('style','color:red');
			}
		},
		computed:{
