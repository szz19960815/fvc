...
//			  	store.dispatch('listAction');
			...
			loadMore(){
				...
				this.timer = setTimeout(()=>{
					let page = store.state.IMG.length/20;
//					console.log(page)
					store.dispatch('moreImagesAction',page+1);
					this.loadLogo = false;
				},800)
			},
			imagesInfo(id){
				document.getElementById("img_info").style.display="block";
				store.state.IMG.map((data,i)=>{
					if(data.id === id){
//						console.log(data.name)
						this.Img = store.state.IMG[i]
//						console.log(this.Img)
			...
			zan(index,e){
				e = event || ''
				store.state.VIDEO[index].love = Number(store.state.VIDEO[index].love)+1;
				e.target.setAttribute('style','color:red');
				store.state.like.push(store.state.VIDEO[index])
			},
			cai(index,e){
				e = event || ''
				store.state.VIDEO[index].hate = Number(store.state.VIDEO[index].hate)+1;
				e.target.setAttribute('style','color:red');
			}
		},
		computed:{
